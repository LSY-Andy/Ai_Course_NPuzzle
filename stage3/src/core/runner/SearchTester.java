...
public final class SearchTester {
    ...
    //问题实例读入到ArrayList中
        ...
        for (HeuristicType heuristicType : heuristics) { 
            ...
            if(step==1)
                ...
                solveProblems(problems, feeder.getDsjointDatabase(heuristicType), heuristicType);
            System.out.println();
        }
    }
    ...
    private static void solveProblems(ArrayList<Problem> problems, Searcher searcher, HeuristicType heuristicType) {
        ...
        for (Problem problem : problems) {
            ...
            String result = "During  " + time1 + " s. " +
                    "we explore " + searcher.nodesExpanded() + " nodes, " +
            ...
            System.out.println("启发函数：" + heuristicType + "，解路径长度：" + path.size() + "，执行了" + time1 + "s，" +"共生成了" + searcher.nodesGenerated() + "个结点，"+
                    "扩展了" + searcher.nodesExpanded() + "个结点");
        }
    }
    /**
