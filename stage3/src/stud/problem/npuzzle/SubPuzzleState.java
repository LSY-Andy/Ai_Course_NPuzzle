...
import stud.g05.solver.DisjointDatabaseBuilder;
        ...
        if(size == 3){
            for (int i = 0; i < n; i++) {
                hash += (points[i].getRow() * size + points[i].getCol()) *
                        Math.pow(size * size, DisjointDatabaseBuilder.positions3[points[i].getVal()]);
            }
        }else{
            for (int i = 0; i < n; i++) {
                hash += (points[i].getRow() * size + points[i].getCol()) *
                        Math.pow(size * size, DisjointDatabaseBuilder.positions4[points[i].getVal()]);
            }
        }
        return hash;
